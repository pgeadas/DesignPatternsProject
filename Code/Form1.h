...
private: System::Void Form1_Paint(System::Object^  sender, System::Windows::Forms::PaintEventArgs^  pe) {
	 ...
	 //draw shield resource
	this->shield->Accept(actualVisitor);
	shield->Notify();


	//calls visitor, do draw the player spaceship
...
private: System::Void Key_Down(System::Object^  sender, System::Windows::Forms::KeyEventArgs^  e) {
		 switch(e->KeyCode)
	 {
		...
		case Keys::C:
			...
			color++;
			 break;
		case Keys::L: //switch among different vistor types
			if(detail%3 == 2){
...
//		simpleBullet = gcnew DrawWhiteBulletSimpleAlgorithm();
		...
		for(int i=0;i<ENEMIES;i++){
			...
			p->Attach(static_cast<Observer^>(cpu)); //attach of enemy spaceships into player observer
			CPUPlayers->Add(cpu);
		}
		...
		for(int i=0;i<ENEMIES;i++){
			...
			for(int j=0;j<ENEMIES;j++){
				if(j!=i){
					cpu->Attach(static_cast<Observer^>(CPUPlayers[j])); //attach of enemy spaceships into eachother, to detect collisions among them
				}
			}
		} 
 }
